...
/// Az első két teszteset a Kifli objektum kiírási formátumát teszteli.
    ...
    TEST(kifli, print2) {
        ...
        std::stringstream ss;
        ss << sajtoskifli;          // inserternek i mennie kell
...
/// Itt pedig csalunk: előredátumozzuk az eladásokat
    p1.elad(2, tej15, Datum(2070, 4, 1));
    p1.elad(2, tej15lm, Datum(2070, 4, 1));
#ifdef KIFLI_IS_VAN
    p1.elad(10, sajtoskifli, Datum(2070, 4, 1));

/// Kell még két kifli?
    if (ennyi_kiflit % 2 == 1)
        p1.elad(2, tejeskifli, Datum(2070, 4, 1));
#endif

    p1.elad(3, tej28);      /// még három liter tejet is eladunk
...
/// Mai eladások
    cout << "\nEladasok (" << Datum(2070, 4,1 ) << "): " << endl;
    p1.list(cout, Datum(2070, 4, 1));

/// Gyenge ellenőrzés: csak a sorok számát ellenőrizzük
    TEST(Kassza, jovore_sorok_szama) {
        std::stringstream ss;
        p1.list(ss, Datum(2070, 4, 1));     // stringstrem-be listázunk
