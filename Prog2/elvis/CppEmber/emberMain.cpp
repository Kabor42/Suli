 ...
 * Ezt legkényelmesebben projekt szintû beállítással tudja elérni.
...
#include "hallgato.h"
#include "kutato.h"
#include "gtest_lite.h" // tesztelést segítõ makrók
#include "memtrace.h"
...
// Segédfüggvény a beolvasások elõtti üzenetek kiírásához
...
// Ember referenciát kap. kiír és ellenõriz
...
// Ember pointert kap. kiír és ellenõriz
...
void test_2(int ma) {
    TEST(tesz2, kiir) {
      ...
      kiirEll(butus, ma, "Buta Toni", "Hallgato", 1975); // kiírja és ellenõrzi az adatokat
    } ENDM
}

// másoló konstruktor és a kivételkezelés mûködésének ellenõrzése
void test_3(int ma) {
    TEST(test3, kiir) {
      ...
      Ember *okos = &valaki;
      kiirEllp(okos, ma, "Okos Toni", "Hallgato", 1970); // kiírja és ellenõrzi az adatokat
