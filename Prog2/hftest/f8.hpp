...
private:
    ...
    size_t curr;                                   /** Current occupation */
    size_t size;                                   /** Maximum size, also class template param */
public:
    F8() {                                      /** CTor */
        curr=0;
        size=T;
 ...
 */
template <size_t N> std::ostream& operator << (std::ostream& os, const F8<N> &v) {
    // 23. 22. 85. 84. 418.
    os << std::fixed;
